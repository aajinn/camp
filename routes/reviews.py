...
def create_review():
    ...
    try:
        ...
        booking = Booking.query.filter_by(
            user_id=user_id, campsite_id=campsite_id, status="paid"
        ).first()
        ...
        if not booking:
            return (
                jsonify(
                    {
                        "error": "You can only review campsites you have booked and paid for"
                    }
                ),
                403,
            )
        ...
        existing_review = Review.query.filter_by(
            user_id=user_id, campsite_id=campsite_id
        ).first()
        ...
        # Create review
        review = Review(
            user_id=user_id, campsite_id=campsite_id, rating=rating, comment=comment
        )
...
def get_campsite_reviews(campsite_id):
    ...
    try:
        ...
        reviews = (
            Review.query.filter_by(campsite_id=campsite_id)
...
def update_review(review_id):
    ...
    try:
        ...
        if review.user_id != user_id:
            return (
                jsonify({"error": "Only the review author can update this review"}),
                403,
            )
...
def delete_review(review_id):
    ...
    try:
        ...
        if review.user_id != user_id:
            return (
                jsonify({"error": "Only the review author can delete this review"}),
                403,
            )
